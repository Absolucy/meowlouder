...
use meowlouder_opus_sys::{
	opus_encode, opus_encode_float, opus_encoder_ctl, opus_encoder_get_size, opus_encoder_init,
	OPUS_GET_BANDWIDTH_REQUEST, OPUS_GET_PACKET_LOSS_PERC_REQUEST, OPUS_GET_SAMPLE_RATE_REQUEST,
	OPUS_RESET_STATE, OPUS_SET_PACKET_LOSS_PERC_REQUEST,
};
...
impl OpusEncoder {
	...
	pub fn encode<Pcm>(&mut self, pcm: Pcm, frame_size: usize) -> Result<Vec<u8>, OpusErrorCode>
	where
		Pcm: AsRef<[i16]>,
	{
	...
	pub fn encode_float_into<Pcm, Data>(
		&mut self,
		pcm: Pcm,
		frame_size: usize,
		mut data: Data,
	...
	{
		let mut data = vec![0; MAX_DATA_BYTES];
		let len = self.encode_float_into(pcm, frame_size, &mut data)?;
		data.truncate(len);
		Ok(data)
	}
